...
async def job_econ_alerts(bot: Bot):
    ...
    for ev in econ:
        ...
        if 3300 < delta < 3900:  # ~55-65 min before
            text = (
                f"⚡️ *تنبيه اقتصادي قادم خلال ساعة!*\n\n"
                f"📌 *{ev.get('name', 'حدث اقتصادي')}*\n"
                f"🕐 الوقت: {ev['time'].strftime('%H:%M')} (المغرب)\n"
                f"💥 التأثير: {ev.get('impact', 'عالي')}\n"
                f"📊 التوقعات: {ev.get('forecast', 'غير محدد')}\n"
                f"📈 السابق: {ev.get('previous', 'غير محدد')}\n\n"
                f"⚠️ كن مستعداً — قد يتحرك السوق بشكل حاد!"
            )
            await broadcast(bot, text, "notify_econ")
            logger.info(f"Econ alert sent: {ev.get('name', '')}")
